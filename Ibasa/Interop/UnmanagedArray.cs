...
namespace Ibasa.Interop
{
        ...
        where T : struct
    {
        ...
        public UnmanagedArray(IEnumerable<T> collection)
        {
            int count;
            if (collection is IList<T>)
                count = (collection as IList<T>).Count;
            else
            ...
            {
                T v = value;
                Memory.Write<T>(Pointer + offset, ref v);
                offset += SizeOfT;
            }
        }
        ...
        public void CopyTo(UnmanagedArray<T> array)
        {
            CopyTo(0, array, 0, Count);
        }

        public void CopyTo(UnmanagedArray<T> array, int arrayIndex, int count)
        {
            CopyTo(0, array, arrayIndex, count);
        }

        public void CopyTo(int index, UnmanagedArray<T> array, int arrayIndex, int count)
        {
            if (index < 0)
                throw new ArgumentOutOfRangeException("index", index, "index is less than zero.");
            if (index + count > Count)
                throw new ArgumentOutOfRangeException("count", count, "index + count is greater than Count.");

            if (array == null)
                throw new ArgumentNullException("array");
            if (arrayIndex < 0)
                throw new ArgumentOutOfRangeException("arrayIndex", arrayIndex, "arrayIndex is less than zero.");
            if (arrayIndex + count > array.Count)
                throw new ArgumentOutOfRangeException("count", count, "arrayIndex + count is greater than array.Count.");

            Memory.Copy(
                Pointer + index * SizeOfT,
                array.Pointer + arrayIndex * SizeOfT,
                count * SizeOfT);
            ++array.Version;
        }

        public UnmanagedArray<T> Subarray(int index, int count)
        {
            int offset = index * SizeOfT;
            int size = count * SizeOfT;
            return new UnmanagedArray<T>(Pointer + offset, size, false);
        }
