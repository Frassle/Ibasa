...
namespace OpenTK.Audio.OpenAL
{
    ...
    public partial class EffectsExtension
    {
        ...
        [CLSCompliant(false)]
        public void Effect(uint eid, EfxEffect3f param, ref Vector3f values)
        {
            unsafe
            {
                ...
                values = new Vector3f(ptr[0], ptr[1], ptr[2]);
            }
        }
        ...
        public void Effect(int eid, EfxEffect3f param, ref Vector3f values)
        {
        ...
        [CLSCompliant(false)]
        public void GetEffect(uint eid, EfxEffect3f param, out Vector3f values)
        {
            unsafe
            {
                float* ptr = stackalloc float[3];
        ...
        public void GetEffect(int eid, EfxEffect3f param, out Vector3f values)
        {
        ...
        public EffectsExtension()
        {
            ...
            if(Alc.GetCurrentContext() == IntPtr.Zero)
                throw new InvalidOperationException("AL.LoadAll() needs a current AudioContext.");

            if(!Alc.IsExtensionPresent(Alc.GetContextsDevice(Alc.GetCurrentContext()), "ALC_EXT_EFX"))
            {
                Debug.Print("EFX Extension (ALC_EXT_EFX) is not supported(AudioContext: {0}).", Alc.GetCurrentContext());
                return;
            }
