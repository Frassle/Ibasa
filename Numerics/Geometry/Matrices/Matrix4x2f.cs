...
{
	/// <summary>
	/// Represents a 4 by 2 matrix of floats.
	/// </summary>
	[Serializable]
	[ComVisible(true)]
	[StructLayout(LayoutKind.Sequential)]
	public struct Matrix4x2f: IEquatable<Matrix4x2f>, IFormattable
	{
		#region Constants
		/// <summary>
		/// Returns a new <see cref="Matrix4x2f"/> with all of its elements equal to zero.
		/// </summary>
		public static readonly Matrix4x2f Zero = new Matrix4x2f(0);
		/// <summary>
		/// Returns a new <see cref="Matrix4x2f"/> with all of its elements equal to one.
		/// </summary>
		public static readonly Matrix4x2f One = new Matrix4x2f(1);
		#endregion
		#region Fields
		/// <summary>
		...
		public float this[int row, int column]
		{
			...
			{
				if (row < 0 || row > 3)
					throw new ArgumentOutOfRangeException("row", "Rows for Matrix4x2f run from 0 to 3, inclusive.");
				if (column < 0 || column > 1)
					throw new ArgumentOutOfRangeException("column", "Columns for Matrix4x2f run from 0 to 1, inclusive.");
		...
		public float this[int index]
		{
			...
			{
				if (index < 0 || index > 7)
					throw new ArgumentOutOfRangeException("index", "Indices for Matrix4x2f run from 0 to 7, inclusive.");
				switch (index)
				{
					case 0: return M11;
					case 1: return M12;
		...
		public Matrix4x2f(Vector2f row1, Vector2f row2, Vector2f row3, Vector2f row4)
		{
			M11 = row1.X;
			M12 = row1.Y;
			M21 = row2.X;
		...
		public static Matrix4x2f operator *(Matrix4x2f matrix, float scalar)
		{
			return Matrix.Multiply(matrix, scalar);
		}
		public static Matrix4x2f operator *(float scalar, Matrix4x2f matrix)
		{
			return Matrix.Multiply(matrix, scalar);
		}
		...
		#region Conversions
		/// <summary>
		/// Defines an explicit conversion of a Matrix4x2d value to a Matrix4x2f.
		/// </summary>
		/// <param name="value">The value to convert to a Matrix4x2f.</param>
		/// <returns>A Matrix4x2f that has all components equal to value.</returns>
		public static explicit operator Matrix4x2f(Matrix4x2d value)
		{
			return new Matrix4x2f((float)value.M11, (float)value.M12, (float)value.M21, (float)value.M22, (float)value.M31, (float)value.M32, (float)value.M41, (float)value.M42);
		}
		#endregion
		#region Equatable
		/// <summary>
		/// Returns the hash code for the current <see cref="Matrix4x2f"/>.
		/// </summary>
		...
		/// <returns>true if the obj parameter is a <see cref="Matrix4x2f"/> object or a type capable
		/// of implicit conversion to a <see cref="Matrix4x2f"/> object, and its value
		/// is equal to the current <see cref="Matrix4x2f"/> object; otherwise, false.</returns>
		public override bool Equals(object obj)
		{
			if (obj is Matrix4x2f) { return Equals((Matrix4x2f)obj); }
			return false;
		}
		/// <summary>
		/// Returns a value that indicates whether the current instance and a specified
		...
		public static bool Equals(Matrix4x2f left, Matrix4x2f right)
		{
			return left == right;
		}
		/// <summary>
	...
	{
		#region Operations
		public static Matrix4x2f Negate(Matrix4x2f value)
		{
			return new Matrix4x2f(-value.M11, -value.M12, -value.M21, -value.M22, -value.M31, -value.M32, -value.M41, -value.M42);
		}
		public static Matrix4x2f Add(Matrix4x2f left, Matrix4x2f right)
		{
			return new Matrix4x2f(left.M11 + right.M11, left.M12 + right.M12, left.M21 + right.M21, left.M22 + right.M22, left.M31 + right.M31, left.M32 + right.M32, left.M41 + right.M41, left.M42 + right.M42);
		}
		public static Matrix4x2f Subtract(Matrix4x2f left, Matrix4x2f right)
		{
			return new Matrix4x2f(left.M11 - right.M11, left.M12 - right.M12, left.M21 - right.M21, left.M22 - right.M22, left.M31 - right.M31, left.M32 - right.M32, left.M41 - right.M41, left.M42 - right.M42);
		}
		public static Matrix4x2f Multiply(Matrix4x2f left, Matrix2x2f right)
		{
			return new Matrix4x2f(left.M11 * right.M11 + left.M12 * right.M21, left.M11 * right.M12 + left.M12 * right.M22, left.M21 * right.M11 + left.M22 * right.M21, left.M21 * right.M12 + left.M22 * right.M22, left.M31 * right.M11 + left.M32 * right.M21, left.M31 * right.M12 + left.M32 * right.M22, left.M41 * right.M11 + left.M42 * right.M21, left.M41 * right.M12 + left.M42 * right.M22);
		}
		public static Matrix4x3f Multiply(Matrix4x2f left, Matrix2x3f right)
		{
		...
		public static Matrix4x4f Multiply(Matrix4x2f left, Matrix2x4f right)
		{
			return new Matrix4x4f(left.M11 * right.M11 + left.M12 * right.M21, left.M11 * right.M12 + left.M12 * right.M22, left.M11 * right.M13 + left.M12 * right.M23, left.M11 * right.M14 + left.M12 * right.M24, left.M21 * right.M11 + left.M22 * right.M21, left.M21 * right.M12 + left.M22 * right.M22, left.M21 * right.M13 + left.M22 * right.M23, left.M21 * right.M14 + left.M22 * right.M24, left.M31 * right.M11 + left.M32 * right.M21, left.M31 * right.M12 + left.M32 * right.M22, left.M31 * right.M13 + left.M32 * right.M23, left.M31 * right.M14 + left.M32 * right.M24, left.M41 * right.M11 + left.M42 * right.M21, left.M41 * right.M12 + left.M42 * right.M22, left.M41 * right.M13 + left.M42 * right.M23, left.M41 * right.M14 + left.M42 * right.M24);
		}
		public static Matrix4x2f Multiply(Matrix4x2f matrix, float scalar)
		{
			return new Matrix4x2f(matrix.M11 * scalar, matrix.M12 * scalar, matrix.M21 * scalar, matrix.M22 * scalar, matrix.M31 * scalar, matrix.M32 * scalar, matrix.M41 * scalar, matrix.M42 * scalar);
		}
		public static Matrix4x2f Divide(Matrix4x2f matrix, float scalar)
		{
			return new Matrix4x2f(matrix.M11 / scalar, matrix.M12 / scalar, matrix.M21 / scalar, matrix.M22 / scalar, matrix.M31 / scalar, matrix.M32 / scalar, matrix.M41 / scalar, matrix.M42 / scalar);
		}
		#endregion
		...
		public static bool All(Matrix4x2f value)
		{
		...
		public static bool All(Matrix4x2f value, Predicate<float> predicate)
		{
		...
		public static bool Any(Matrix4x2f value)
		{
			return value.M11 != 0 || value.M12 != 0 || value.M21 != 0 || value.M22 != 0 || value.M31 != 0 || value.M32 != 0 || value.M41 != 0 || value.M42 != 0;
		}
		/// <summary>
		...
		/// predicate; otherwise, false.</returns>
		public static bool Any(Matrix4x2f value, Predicate<float> predicate)
		{
			return predicate(value.M11) || predicate(value.M12) || predicate(value.M21) || predicate(value.M22) || predicate(value.M31) || predicate(value.M32) || predicate(value.M41) || predicate(value.M42);
		}
		#endregion
		...
		public static Matrix4x2d Transform(Matrix4x2f value, Func<float, double> transformer)
		{
		...
		/// <param name="transformer">A transform function to apply to each element.</param>
		/// <returns>The result of transforming each element of value.</returns>
		public static Matrix4x2f Transform(Matrix4x2f value, Func<float, float> transformer)
		{
		...
		public static Matrix4x2f Modulate(Matrix4x2f left, Matrix4x2f right)
		{
			return new Matrix4x2f(left.M11 * right.M11, left.M12 * right.M12, left.M21 * right.M21, left.M22 * right.M22, left.M31 * right.M31, left.M32 * right.M32, left.M41 * right.M41, left.M42 * right.M42);
		}
		/// <summary>
		...
		{
			return new Matrix4x2f(Functions.Abs(value.M11), Functions.Abs(value.M12), Functions.Abs(value.M21), Functions.Abs(value.M22), Functions.Abs(value.M31), Functions.Abs(value.M32), Functions.Abs(value.M41), Functions.Abs(value.M42));
		}
		/// <summary>
		...
		{
			return new Matrix4x2f(Functions.Saturate(value.M11), Functions.Saturate(value.M12), Functions.Saturate(value.M21), Functions.Saturate(value.M22), Functions.Saturate(value.M31), Functions.Saturate(value.M32), Functions.Saturate(value.M41), Functions.Saturate(value.M42));
		}
		/// <summary>
		...
		{
			return new Matrix4x2f(Functions.Ceiling(value.M11), Functions.Ceiling(value.M12), Functions.Ceiling(value.M21), Functions.Ceiling(value.M22), Functions.Ceiling(value.M31), Functions.Ceiling(value.M32), Functions.Ceiling(value.M41), Functions.Ceiling(value.M42));
		}
		/// <summary>
		...
		public static Matrix4x2f Floor(Matrix4x2f value)
		{
			return new Matrix4x2f(Functions.Floor(value.M11), Functions.Floor(value.M12), Functions.Floor(value.M21), Functions.Floor(value.M22), Functions.Floor(value.M31), Functions.Floor(value.M32), Functions.Floor(value.M41), Functions.Floor(value.M42));
		}
		/// <summary>
		...
		{
			return new Matrix4x2f(Functions.Truncate(value.M11), Functions.Truncate(value.M12), Functions.Truncate(value.M21), Functions.Truncate(value.M22), Functions.Truncate(value.M31), Functions.Truncate(value.M32), Functions.Truncate(value.M41), Functions.Truncate(value.M42));
		}
		/// <summary>
		...
		public static Matrix4x2f Fractional(Matrix4x2f value)
		{
			return new Matrix4x2f(Functions.Fractional(value.M11), Functions.Fractional(value.M12), Functions.Fractional(value.M21), Functions.Fractional(value.M22), Functions.Fractional(value.M31), Functions.Fractional(value.M32), Functions.Fractional(value.M41), Functions.Fractional(value.M42));
		}
		/// <summary>
		...
		{
			return new Matrix4x2f(Functions.Round(value.M11), Functions.Round(value.M12), Functions.Round(value.M21), Functions.Round(value.M22), Functions.Round(value.M31), Functions.Round(value.M32), Functions.Round(value.M41), Functions.Round(value.M42));
		}
		/// <summary>
		/// Returns a matrix where each element is rounded to the nearest integral value.
		/// </summary>
		...
		{
			return new Matrix4x2f(Functions.Round(value.M11, digits), Functions.Round(value.M12, digits), Functions.Round(value.M21, digits), Functions.Round(value.M22, digits), Functions.Round(value.M31, digits), Functions.Round(value.M32, digits), Functions.Round(value.M41, digits), Functions.Round(value.M42, digits));
		}
		/// <summary>
		/// Returns a matrix where each element is rounded to the nearest integral value.
		/// </summary>
		...
		{
			return new Matrix4x2f(Functions.Round(value.M11, mode), Functions.Round(value.M12, mode), Functions.Round(value.M21, mode), Functions.Round(value.M22, mode), Functions.Round(value.M31, mode), Functions.Round(value.M32, mode), Functions.Round(value.M41, mode), Functions.Round(value.M42, mode));
		}
		/// <summary>
		/// Returns a matrix where each element is rounded to the nearest integral value.
		/// </summary>
		...
		/// <param name="mode">Specification for how to round value if it is midway between two other numbers.</param>
		/// <returns>The result of rounding value.</returns>
		public static Matrix4x2f Round(Matrix4x2f value, int digits, MidpointRounding mode)
		{
			return new Matrix4x2f(Functions.Round(value.M11, digits, mode), Functions.Round(value.M12, digits, mode), Functions.Round(value.M21, digits, mode), Functions.Round(value.M22, digits, mode), Functions.Round(value.M31, digits, mode), Functions.Round(value.M32, digits, mode), Functions.Round(value.M41, digits, mode), Functions.Round(value.M42, digits, mode));
		}
		/// <summary>
		...
		public static Matrix4x2f Reciprocal(Matrix4x2f value)
		{
			return new Matrix4x2f(1 / value.M11, 1 / value.M12, 1 / value.M21, 1 / value.M22, 1 / value.M31, 1 / value.M32, 1 / value.M41, 1 / value.M42);
		}
		#endregion
		#region Submatrix
		/// <summary>
		...
		public static Vector3f Submatrix(Matrix4x2f matrix, int row, int column)
		{
			if (row < 0 || row > 3)
				throw new ArgumentOutOfRangeException("row", "Rows for Matrix4x2f run from 0 to 3, inclusive.");
			if (column < 0 || column > 1)
			...
			if (row == 0 && column == 0)
			{
				return new Vector3f(matrix.M22, matrix.M32, matrix.M42);
			}
			else if (row == 0 && column == 1)
			{
				return new Vector3f(matrix.M21, matrix.M31, matrix.M41);
			}
			else if (row == 1 && column == 0)
			{
				return new Vector3f(matrix.M12, matrix.M32, matrix.M42);
			}
			else if (row == 1 && column == 1)
			{
				return new Vector3f(matrix.M11, matrix.M31, matrix.M41);
			}
			else if (row == 2 && column == 0)
			{
				return new Vector3f(matrix.M12, matrix.M22, matrix.M42);
			}
			else if (row == 2 && column == 1)
			{
				return new Vector3f(matrix.M11, matrix.M21, matrix.M41);
			}
			else if (row == 3 && column == 0)
			{
				return new Vector3f(matrix.M12, matrix.M22, matrix.M32);
			}
			else
			{
				return new Vector3f(matrix.M11, matrix.M21, matrix.M31);
			}
		}
		#endregion
		#region Transpose
		...
		public static Matrix2x4f Transpose(Matrix4x2f matrix)
		{
			return new Matrix2x4f(matrix.M11, matrix.M21, matrix.M31, matrix.M41, matrix.M12, matrix.M22, matrix.M32, matrix.M42);
		}
		#endregion
	}
}
